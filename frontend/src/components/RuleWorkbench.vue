...
const fields = [{"key":"productNo","label":"产品编号","type":"text"},{"key":"demand","label":"需求量","type":"number"},{"key":"supply","label":"已确认供应","type":"number"},{"key":"onHand","label":"现有库存","type":"number"},{"key":"safetyStock","label":"安全库存","type":"number"},{"key":"capacity","label":"可用产能","type":"number"},{"key":"unitRevenue","label":"单位收入","type":"number"},{"key":"unitCost","label":"单位成本","type":"number"}]
const form = reactive({"productNo":"P-001","demand":1000,"supply":100,"onHand":100,"safetyStock":100,"capacity":700,"unitRevenue":20,"unitCost":12})
...
async function run() {
  busy.value = true
  error.value = ''
  try { result.value = await request('/api/enterprise/ibp/balance-plan', { method: 'POST', body: JSON.stringify(payload()) }) }
...
<template>
  <section class="rule-head">
    <div><span>DOMAIN CONTROL</span><h3>供需与财务平衡试算</h3><p>在库存、供应和产能约束下计算生产建议、缺口、服务水平与利润。</p></div>
    <b>服务端强校验</b>
  </section>
